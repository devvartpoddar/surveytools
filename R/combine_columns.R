...
#' @param type one of "starts_with", "ends_with", "matches", "contains", "one_of". Defaults to starts_with # nolint
#'
#' @return TBU
#'
#' @importFrom magrittr "%>%"
#'
#' @export
combine_columns <- function(.data, new_column_name, pattern, type = "starts_with") { # nolint
  UseMethod("combine_columns")
}

#' @rdname combine_columns
#' @export
combine_columns.default <- function(.data, new_column_name, pattern, type = "starts_with") { # nolint
  # Capturing the selected column name and selecting names
  quo_column_name <- rlang::enquo(new_column_name)
  selecting_function <- switch(type,
  ...
  .data %>%
    ...
    dplyr::mutate_at(dplyr::across(selecting_function(pattern), ~ ifelse(is.na(.), "", .))) %>% # nolint
