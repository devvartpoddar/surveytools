...
#' @param stat One of "mean", "median" or "total". Defaults to mean if not correctly provided # nolint
#' @param compare Stat to use for significance tests of survey results. Must be provided in "group::response" format. Defaults to the overall value # nolint
#' @param survey.design List of survey design inputs to be applied to dataframe, if not survey data of class 'tbl_svy'. Please see '?survey::svydesign' to identify required inputs # nolint
#' @param ci_level confidence level to be summarised for
#' @param simplify Should the survey results be simplified from decimals to numbers? # nolint
#'
...
summariser_base.default <- function(.data,
                                    ...
                                    # CI level to be used
                                    ci_level = 0.95,
                                    # Simplify results by converting to percentage # nolint
                                    simplify = TRUE) {
  ...
  if (class(tmp) == "try-error") {
    rlang::abort(glue::glue("Column {rlang::quo_text(enquo_var)} does not exist in the dataframe")) # nolint
  }
  ...
  if (class(tmp) == "try-error") {
    rlang::abort(glue::glue("Grouping vars provided do not exist in the dataframe. Please check again")) # nolint
  }
  ...
  if (!purrr::is_character(compare)) {
    rlang::abort("`compare` can only take a string of the form {group}::{response} in order to estimate significance") # nolint
  }
  ...
  if (!is.null(survey.design)) {
    ...
    argsList <- c(
      "ids", "probs", "strata", "fpc",
      "weights", "nest", "check.strata"
    )
    if (all(names(survey.design) %in% argsList)) {
      rlang::abort("List of inputs provided in survey design do not match inputs. Please see '?survey::svydesign' to identify required inputs") # nolint
    }

    rm(argsList)

    # 6b | Check if variables provided in survey.design are present in the dataframe # nolint
    # TODO
  ...
  if (!inherits(.data, "tbl_svy")) {
    rlang::abort("Data is not defined as a survey data of class 'tbl_svy'. Please provide a survey.design, or data set up as a survey") # nolint
  }

  # 8 | Checks for significance numbers and values
  # 8a | Is the CI level numeric
  if (!is.numeric(ci_level)) {
    rlang::warn("Please provide a numeric value for ci_level. Defaulting to 0.95") # nolint
  ...
  if (length(ci_level) > 1) {
    rlang::warn("More than one CI levels are provided. Defaulting to first value") # nolint
  ...
  # Test significance ----
  .data <- test_significance(.data, compare) # nolint
  ...
  if (simplify) {
    ...
    if (!IS_NUMERIC &
      stat != "total") {
      .data <- .data %>%
        dplyr::mutate(dplyr::across(starts_with("value"), ~ percent(.x, digits = 2))) # nolint
    } else {
      .data <- .data %>%
...
summariser_numeric <- function(.data, var, group, stat, level) {
  ...
  if (lgl) {
    ...
    rlang::inform(glue::glue("Variable {var} has been converted from logical to numeric. Please force as character if you wish to keep both categories")) # nolint
  }
  ...
  if (length(group) > 0) {
    ...
    grouped_data <- purrr::map_dfr(group, function(group_var) {
      .data %>%
        ...
        srvyr::select(group, response, N, value, everything(), -!!rlang::sym(group_var)) # nolint
    })
...
summariser_character <- function(.data, var, group, stat, level) {
  ...
  if (stat == "median") {
    rlang::warn("`stat` median is not allowed with character vectors, providing survey mean instead") # nolint
  }

  # Sanity checks
  # 1. Converting NAs in var and group var to character
  .data <- .data %>%
    srvyr::mutate(dplyr::across(dplyr::any_of(c(var, group)), stringr::str_replace_na)) # nolint
    ...
    grouped_data <- purrr::map_dfr(group, function(group_var) {
      ...
      purrr::map_dfr(list_of_responses, function(x) {
        .data %>%
          ...
          srvyr::mutate(
            group = case_when(
              # If more than one groups are provided, attach a group name finder, else just the normal works # nolint
              length(group) > 1 ~ paste0(group_var, " | ", !!rlang::sym(group_var)), # nolint
